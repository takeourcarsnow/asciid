...
import { SpatialDistanceCache, NoiseCache, sphereIntersectsBound } from './optimization';
...
export function startRenderer(canvas: HTMLCanvasElement, viewer: HTMLElement){
  ...
  // Performance optimization: Initialize spatial and noise caches
  ...
  function render(){
    ...
    if(state.adaptive && state.noiseEnabled && frame % 10 === 0){ const target = state.targetFps; const headroom = fpsEMA - target; if(headroom < -2 && state.noiseOct > 1){ state.noiseOct = Math.max(1, state.noiseOct - 1); setValue('noiseOct', state.noiseOct); } else if(headroom > 6 && state.noiseOct < 5){ state.noiseOct = Math.min(5, state.noiseOct + 1); setValue('noiseOct', state.noiseOct); } }
