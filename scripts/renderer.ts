...
export function startRenderer(canvas: HTMLCanvasElement, viewer: HTMLElement){
  ...
  function render(){
    ...
    if(state.autoSpin){ state.rotY += state.spinSpeed; if(state.rotY>180) state.rotY-=360; if(state.rotY<-180) state.rotY+=360; }
    ...
    ctx.save(); ctx.fillStyle = '#000'; ctx.fillRect(0,0, canvas.width/dpr, canvas.height/dpr); ctx.restore();
